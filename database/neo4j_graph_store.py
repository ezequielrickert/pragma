...
# (docs/dev/database/neo4j_graph_store.md#module) moved their real
# definitions to `_neo4j_cypher_helpers.py`.

# Silences a harmless WARNING seen on a fresh site's first pages.
# Details: docs/dev/database/neo4j_graph_store.md#logging-silence
...
class Neo4jGraphStore(
    ...
    Details: docs/dev/database/neo4j_graph_store.md#neo4jgraphstore
    """
    ...
    def connect(self) -> None:
        if not self.password:
            ...
            # Details: docs/dev/database/neo4j_graph_store.md#connect-no-password
            raise RuntimeError(
    ...
    def get_page_measurements(self, site: str) -> Dict[str, Any]:
        with self._session() as session:
            result = session.run(
                ...
                RETURN p.url AS url, p.measurements AS measurements
                """,
                site=site,
            )
    ...
    def record_links(self, site: str, from_url: str, links: List[Dict[str, str]]) -> None:
        ...
        Details: docs/dev/database/neo4j_graph_store.md#record_links
        """
        if not links:
            return
    ...
    def clear_site(self, site: str) -> None:
        ...
        # Details: docs/dev/database/neo4j_graph_store.md#clear_site
        with self._session() as session:
