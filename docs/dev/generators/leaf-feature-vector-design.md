...
fixed-length `float` vector per `Component` node, stored as a Kùzu `FLOAT[168]` column, compared
...
| Structural | `component_type` | one-hot, 12 closed values (see below) | 12 | 1.0 |
...
`component_type`'s 12 closed values: the 11 literal strings `classify_component_type()`
(`generators/component_classifier.py`) can return, with its templated
`"text field (<input_type>)"` branch collapsed to one generic `"text field"` value for vector
purposes (the specific `input_type` is already captured by its own hashed field above).
