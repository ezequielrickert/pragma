# `spiders/browser/crawl4ai_crawler.py`
...
## _wait_for_new_content and friends

Moved to `docs/dev/spiders/browser/dom_settle.md` along with the code itself
(`_is_navigation_context_error`, `_adaptive_wait_step_seconds`,
`_stable_hold_seconds`, `_wait_for_new_content`, and the empanad.app timing
table) once `crawl4ai_crawler.py` tripped `file-size-audit`'s SPLIT tier -
see that doc for the full history (two real production bugs, both
live-measured) behind this settle-wait's design.
...
bound already documents in `docs/dev/spiders/orchestration/mechanical_loop.md#_worker`).
...
- `interaction_timeout_seconds`: A *third* timeout phase, distinct from
  ...
  *unbounded-by-default* internal waits inside a single interaction
  ...
  circuit breaker (`docs/dev/spiders/orchestration/page_visitor.md#_max_consecutive_unexplained_failures`)
...
See `docs/dev/spiders/orchestration/mechanical_loop.md#in_flight` for the deeper fix
...
`docs/dev/spiders/orchestration/page_visitor.md#_recover_stale_frontier`): after an
...
3. *Periodically, every `session_recycle_after` visits* - the actual
   ...
   `docs/dev/spiders/orchestration/mechanical_loop.md#_recycle_session_if_due` for
...
`docs/dev/spiders/orchestration/mechanical_loop.md#session_recycle_after` for picking
that number.
