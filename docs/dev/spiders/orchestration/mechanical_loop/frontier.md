...
Re-add a URL an earlier, separate `scout_only` run's own frontier
(`config.md#scout_only`) already fully drained through this same
...
twice" - so `interact_only` needs its own entry point past that one
guard, while still keeping the scope gate (`enqueue-scope-gate` above).
...
scouted set already respects `max_visits_per_route_shape` from that
earlier run's own `enqueue()` gate the first time each URL went through
it.
