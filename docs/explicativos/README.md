...
| [`neo4j.md`](neo4j.md) | Esquema del grafo (nodos, relaciones, `<id>`/`<elementId>`, identidad de URL). | **Vigente** (actualizado en `docs/explicativos/plan-almacenamiento.md` Fase A) — cubre los cuatro tipos de nodo actuales (incluye `TextContent`), `description`/`title`/`network_requests`, y la identidad de URL automática (`clean_url`/`route_shape`). Ante cualquier duda puntual, `src/core/interfaces.py::GraphStore` sigue siendo la fuente de verdad. |
...
| [`plan-almacenamiento.md`](plan-almacenamiento.md) | Investigación y plan de mejoras del área de storage/documentos de resultado (`src/storage/`, `src/generators/`, `docs/`, `debug_logs/`) sobre la arquitectura **vigente** (post-`crawl4ai`). | **Vigente** — documento activo, se actualiza fase a fase con una bitácora de riesgos/aprendizajes. |
