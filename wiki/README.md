...
- "A page keeps getting re-fetched to finish interacting with it, because a nav-link click ends its pass part-way through" → crawl4ai-integration-pitfalls (decline the navigation at the `page.route()` layer instead of coping with it - one URL, one fetch)
