...
class GraphStore(ABC):
    ...
    def get_component_families(self, site: str) -> List[ComponentFamily]:
        ...
        Returns:
            A list of `ComponentFamily` (see `core/interfaces.py`'s
    ...
    # component families above, computed by generators/request_family.py
    ...
    # Structural containment: which real layout/landmark containers (nav,
    # section, aside, ...) a component sits inside - captured by
    # discover_components.js's `structuralAncestorsOf` (Storage Phase 5) so a
    # post-hoc pass can group components into modules instead of only ever
    # seeing a flat component list with no hierarchy at all. Not abstract -
    # a backend with no structural-analysis use for this can ignore it, same
    # reasoning as `record_accessibility_violations`. Both shipped graph
    # backends (Neo4j, DuckDB) and the in-memory reference do implement it.
    # Details: docs/dev/core/interfaces.md#structural-containment
