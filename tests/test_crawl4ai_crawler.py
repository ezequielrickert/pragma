"""Regression tests for Crawl4AICrawler (spiders/browser/crawl4ai_crawler.py),
...
def _discover_many(urls: List[str]):
    async def run():
        async with Crawl4AICrawler(Crawl4AICrawlerConfig(wait_seconds=0)) as crawler:
            return await crawler.discover_pages_many(urls)

    return asyncio.run(run())


def test_discover_pages_many_returns_a_page_state_per_url_in_order(fixture_server):
    """arun_many()'s own internal ordering must not leak through - each
    (url, PageState) pair must line up with the url that was requested,
    not with whatever order crawl4ai happened to finish them in. Also
    regression-covers a real bug found while building this method: a
    per-url CrawlerRunConfig with no url_matcher set matches every URL, so
    arun_many() silently bound both pages to the first config's session_id
    and one page's extraction went missing entirely."""
    urls = [f"{fixture_server}/index.html", f"{fixture_server}/image_page.html"]
    pages = _discover_many(urls)

    assert [url for url, _ in pages] == urls
    index_state = pages[0][1]
    image_state = pages[1][1]
    assert index_state.title == "Spike Fixture"
    assert any(c["text"] == "Escaped id button" for c in index_state.components)
    assert any(c["text"] == "Click me" for c in image_state.components)


def test_discover_pages_many_reports_a_failed_page_as_none_without_costing_the_rest(tracking_fixture_server):
    """One page timing out must not raise and must not take the batch's
    other pages down with it - the whole reason this method has a
    (url, Optional[PageState]) contract instead of discover_page's
    raise-on-failure one."""
    base_url, _ = tracking_fixture_server
    slow_url = f"{base_url}/slow.html"
    index_url = f"{base_url}/index.html"

    async def run():
        async with Crawl4AICrawler(Crawl4AICrawlerConfig(wait_seconds=0, page_timeout_seconds=1)) as crawler:
            return await crawler.discover_pages_many([slow_url, index_url])

    pages = asyncio.run(run())
    pages_by_url = dict(pages)
    assert pages_by_url[slow_url] is None
    assert any(c["text"] == "Escaped id button" for c in pages_by_url[index_url].components)


def test_discover_pages_many_with_no_urls_returns_empty_without_navigating():
    assert _discover_many([]) == []
