...
def test_connect_writes_a_site_header_row_once(tmp_path, site) -> None:
    ...
    store.connect()
    try:
        rows = store._call(lambda conn: list(_rows(conn.execute("MATCH (s:Site) RETURN s.name"))))
        assert rows == [[site]]
    finally:
        store.close()
...
def test_connect_is_idempotent_and_never_duplicates_the_site_row(tmp_path, site) -> None:
    ...
    try:
        count = store._call(lambda conn: list(_rows(conn.execute("MATCH (s:Site) RETURN count(s)"))))[0][0]
        assert count == 1
    finally:
        store.close()
...
def test_touch_site_keeps_first_crawled_but_advances_last_crawled(tmp_path, site) -> None:
    ...
    try:
        first = store._call(
            lambda conn: list(_rows(conn.execute("MATCH (s:Site) RETURN s.first_crawled")))
        )[0][0]
        store._touch_site()
        after = store._call(
            lambda conn: list(_rows(conn.execute("MATCH (s:Site) RETURN s.first_crawled, s.last_crawled")))
        )[0]
...
def test_reset_deletes_every_node_but_reopens_with_a_fresh_site_row(tmp_path, site) -> None:
    ...
    try:
        ...
        before = store._call(lambda conn: list(_rows(conn.execute("MATCH (p:Page) RETURN count(p)"))))[0][0]
        assert before == 1

        store.reset()

        after = store._call(lambda conn: list(_rows(conn.execute("MATCH (p:Page) RETURN count(p)"))))[0][0]
        assert after == 0
        site_rows = store._call(lambda conn: list(_rows(conn.execute("MATCH (s:Site) RETURN s.name"))))
...
def test_each_site_gets_its_own_database_no_cross_contamination(tmp_path) -> None:
    ...
    try:
        ...
        a_pages = a._call(lambda conn: list(_rows(conn.execute("MATCH (p:Page) RETURN count(p)"))))[0][0]
        b_pages = b._call(lambda conn: list(_rows(conn.execute("MATCH (p:Page) RETURN count(p)"))))[0][0]
        assert a_pages == 1
        assert b_pages == 0
    finally:
        a.close()
