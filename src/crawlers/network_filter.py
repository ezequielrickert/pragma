...
def filter_meaningful_requests(raw_events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Reduce one `arun()` call's `result.network_requests` (or `None`) to
    the meaningful subset, one dict per kept request:
    `{"method", "url", "resource_type", "status": Optional[int],
    "failed": bool, "failure_text": Optional[str]}`.

    Response BODY TEXT is deliberately never read into the output - it can
    be arbitrarily large (a full JSON payload, or an entire document for a
    ...
    KNOWN LIMITATION: request/response/failure are joined purely by URL, not
    by any per-request id crawl4ai doesn't expose - two requests to the
    ...
    for event in raw_events:
        ...
        if event_type == "response":
            ...
            # The response arrived but its body couldn't be read - still a
            # real response, just with no status captured for this URL.
