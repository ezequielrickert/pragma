...
Nothing here is ever cleaned up automatically - every run gets its own
timestamped directory, forever, unless `prune_old_runs` (below) is opted
into via `PragmaConfig.debug_logs_keep_last`. See
`docs/explicativos/plan-almacenamiento.md` for why unbounded retention was
flagged as a real operability gap, not just a cosmetic one.
"""
