...
Post-crawl4ai-migration: the per-step decision vocabulary that used to live
here (`Action`/`AgentAction`/`TOOL_SPECS`/`parse_agent_action`/`Agent.act()`)
is gone along with the per-step LLM decision loop it served - there is no
longer a numbered "Clickable elements" list for a model to pick from, since
`MechanicalCrawler` (src/crawlers/mechanical_loop.py) interacts with every
discovered element mechanically. `Scraper`/`PRDGenerator` are gone too: they
modeled a synchronous, lazily-started, single-`Page`/single-call-return shape
that doesn't fit `Crawl4AICrawler`'s async, `AsyncWebCrawler`-owns-the-
browser-lifecycle model, or the new crawl()-then-synthesize() split (see
`Engine`). `PageState`, `Agent` (now just `generate()`), and `GraphStore`
remain - the contracts that are still genuinely shared across implementations.
"""
...
class PageState:
    ...
    # Crawl4AICrawler._run_extraction for how this is built and
    ...
    description: str = ""
    # Meaningful (xhr/fetch) network requests triggered by the interaction
    # that produced this PageState - see
    ...
    # visit alongside `components` - see
    # src/crawlers/js/extract_text_content.js for exactly what's captured and
    # excluded (anything that's an interactive component's own label text).
    # Each entry: {tag, text, path, visible, rect}.
...
class GraphStore(ABC):
    ...
    already-normalized, scheme-stripped node key (see
    `src.utils.urls.clean_url`) - the store itself does not re-normalize.
    """
    ...
    def upsert_page(
        ...
        `description`: a short page-level summary (meta description, or
        heading + first paragraph - see `Crawl4AICrawler._run_extraction`'s
        `description` extraction), stored so it survives past the crawling
        process that discovered it. Empty string never overwrites a
        previously-recorded non-empty value, same "don't clobber with less
        information" discipline as `context`/`label` above. Retrieved in bulk
        via `get_page_descriptions`, not per-page, since synthesis reads
        every page's description once at the end of a run.

        `title`: the page's own `<title>` (`PageState.title`, already
        extracted by `Crawl4AICrawler._run_extraction` but previously never
        persisted) - distinct from `label` (the anchor text of whichever link
        happened to lead here, which is absent for the crawl's own start URL
        and can vary per discovering page) and from `description` (a
        sentence-length summary, not a short name). This is what a document
        renderer should show as "the name of this page" - see
        `get_page_titles`. Same empty-string-never-clobbers discipline as
    ...
    def get_page_descriptions(self, site: str) -> Dict[str, str]:
        ...
        (pages with no/empty description are omitted, not included as `""`).
        """
        raise NotImplementedError
    ...
    def get_page_titles(self, site: str) -> Dict[str, str]:
        ...
        renderer reads every page's title once in bulk, not one query per
        page.
        """
        raise NotImplementedError
    ...
    def record_link(self, site: str, from_url: str, to_url: str, label: str) -> None:
        ...
        later navigation's component description can be verified against the
        ...
        than one per from/to pair) previously caused a navigation's reported
    ...
    def get_loop_signals(self, site: str, url: str) -> List[Dict[str, str]]:
        ...
        Empty if `url` has never been reached before.
        """
        raise NotImplementedError
    ...
    def clear_site(self, site: str) -> None:
        ...
        (`PragmaConfig.fresh`) before wiring the crawl. Without it, a site
        ...
        next run's synthesis step still reads back as real history. A
    ...
    # crawler moves on, and never present at all on turn one of a later run
    # against the same persisted site) or isn't tracked at all. `page_url` here is
    # always the already-`clean_url`-canonicalized page key, exactly like every
    # `url` elsewhere in this interface; `path` is the CSS selector the crawler
    # itself produces for the element (its `gp()` helper), reused as-is rather
    ...
    def record_component(
        ...
        `Crawl4AICrawler`'s discovery JS), `None` when unknown. This is what
        makes the stored checklist a *precise* map of the page - not just
        "this exists somewhere" but "this exists right here."
    ...
    def record_component_network(self, site: str, page_url: str, path: str, requests_json: str) -> None:
        ...
        (`src/crawlers/network_filter.py::filter_meaningful_requests`'s
        output for one interaction) to a Component's `network_requests`
        list - same append-only-list-of-JSON-strings shape as
        `record_component_interaction`'s `interactions`, not an overwrite
        like `record_component_options`' `options`: an interaction can only
        happen once per path today (the tracker's consult-before-act guard
        forbids re-interacting an already-interacted path), but modeling
        this as append is safe-by-construction if that invariant ever
        changes, rather than silently losing an earlier batch. Auto-creates
        the Component node if missing, same discipline as
    ...
    def get_component_states(self, site: str, page_url: str) -> Dict[str, Dict[str, Any]]:
        ...
        One query per page visit, not one per component - `GraphStoreInteractionTracker`
        (src/crawlers/graph_sink.py) is the caller. `x`/`y`/`width`/`height`
        are `None` for components recorded before position tracking existed,
        or by a test double that doesn't report it. `options` is the raw JSON
    ...
    def get_component_ledger(self, site: str) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        `network_requests` is a list of `filter_meaningful_requests`-shaped
        dicts (already-decoded, not raw JSON strings) - `[]` if the
        component never triggered a meaningful request or predates this
        field's existence.
    ...
    # exists to eliminate) or every interaction-frontier query needs a
    # kind != 'text' filter bolted on. A second, purpose-built node kind with
    # its own identity keeps the interaction surface untouched by
    # construction.
    ...
    def record_text_content(
        ...
        Called once per page visit (see `MechanicalCrawler._visit_page`),
        *not* re-called on same-page reveals the way `record_inventory` now
        is for `Component` - text revealed only by an interaction is a real,
        structurally-symmetric gap to the ghost-node bug, but out of scope
        for this feature by explicit design.
        """
        raise NotImplementedError

    @abstractmethod
    def get_text_content_ledger(self, site: str) -> Dict[str, List[Dict[str, Any]]]:
        """{page_url: [{"path", "tag", "text", "visible", "x", "y", "width",
        "height"}, ...]} for every text node ever recorded for `site` -
        mirrors `get_component_ledger`'s bulk-per-site shape (one query for
        the whole site, not one per page).
        """
        raise NotImplementedError
