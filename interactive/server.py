...
is one in-memory list per `(filename, extension)`, held in
`app.config["CHAT_HISTORY"]` - gone when the session ends (map #146's
...
def _chat_panel(ref: DocumentRef, history: List[Dict[str, str]], chat_error: Optional[str]) -> str:
    ...
    error here.
    Details: docs/dev/interactive/server.md#_chat_panel
    """
...
def create_app(out_dir: str, site: str, agent: Agent) -> Flask:
    ...
    def edit_document(filename: str, extension: str):
        ...
        body = _document_page(ref, content, error) + _chat_panel(ref, history, None)
        return _page(f"{filename}.{extension} - {site}", body)
    ...
    def chat(filename: str, extension: str):
        ...
        body = _document_page(ref, content, None) + _chat_panel(ref, history, chat_error)
        return _page(f"{filename}.{extension} - {site}", body)
